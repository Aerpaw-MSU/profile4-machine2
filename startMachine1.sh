...
if [ $MODE == "TESTBED" ]
  ...
  screen -S setip -dm bash -c "ifconfig tap2 192.168.0.2"
...
then
  ...
  screen -S setip -dm bash -c "ifconfig tap2 172.17.0.2"
...
# screen -S ping -dm bash -c "
 docker exec -it tests_node_1_1 ping -w 60 172.17.0.2 | ts '[%Y-%m-%d %H:%M:%.S]' > zchemping1.log
 sleep 70
 pkill screen
